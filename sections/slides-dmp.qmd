...
- How and when will data be shared? Are there possible restrictions to data sharing or embargo reasons? (**data access, embargo, archives, ...**)

- How will data for preservation be selected, and where data will be preserved long-term (for example a data repository or archive)? (**repository, metadata standards, ...**)

- What methods or software tools are needed to access and use data? (**mechanism, request handle, ...**)

- How will the application of a unique and persistent identifier (such as a Digital Object Identifier (DOI)) to each dataset be ensured? (*PID with repository linked with data papers, ...*)
...
- Who (for example role, position, and institution) will be responsible for data management (i.e. the data steward)? (**DMP responsible, data stewardship, ...**)

- What resources (for example financial and time) will be dedicated to data management and ensuring that data will be FAIR (Findable, Accessible, Interoperable, Re-usable)? (**data curation, data storage, cost, ...**)
